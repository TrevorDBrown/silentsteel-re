...
def setup_ffplay() -> str:
    ...
    successful_ffmpeg_install, ffmpeg_install_message = local_ffmpeg.install(
        ffplay_path
    )
...
def shutdown_player(ffplay_path: str) -> None:
    return
...
def play_audio(
    active_audio_file: str, audio_parts: dict, idx: int, ffplay_path: str
) -> None:
    audio_params: dict = {}
...
def play_video(
    active_video_file: str, video_parts: dict, idx: int, ffplay_path: str
) -> None:
            ...
            if active_video_file.endswith("MPG"):
                ...
                run(
                    [
                        ffplay_path,
                        "-ss",
                        ms_to_timecode(start),
                        "-t",
                        ms_to_timecode(end - start),
                        active_video_file,
                        "-hide_banner",
                        "-vf",
                        "scale=-1:480",
